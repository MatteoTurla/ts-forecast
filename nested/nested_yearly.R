...
daily_data <- data_ts[data_ts$period == "Quarterly",]
...
data_prep_full_tbl <- daily_data_prep %>%
  ...
  tk_augment_differences(.value = target_lag6,.lags = 1,.differences = 1) %>%
...
rcp_spec <-
  ...
  step_rm(matches("(iso)|(xts)|(minute)|(second)|(date_wday)|(date_wday)|(date_am.pm)|(date_hour)|(date_hour12)|(date_day)|
                  (date_mday7)|(date_mday)|(mweek)|(qday)|(date_year)|(date_half)|(date_quarter)|(date_yday)")) %>%
  step_rm(starts_with("date_week")) %>%
  step_rm(starts_with("date_month")) %>%
  step_dummy(all_nominal(), one_hot = TRUE) %>%
  # Interaction
  # Fourier
  step_fourier(date, period = c(1,2, 4), K = 1) %>%
