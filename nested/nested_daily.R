...
daily_data <- data_ts[data_ts$period == "Daily",]

daily_data %>% count(id, type)

# 20 time series, test horizon = 14 days
# some tie series contains missing values
...
data_prep_full_tbl <- daily_data_prep %>%
  ...
  tk_augment_differences(.value = target_lag14,.lags = 1,.differences = 1) %>%
